...
def calculate_syntactic_complexity_test():
    atomic_term = NALGrammar.Terms.Term.from_string("A")
    atomic_term_complexity = 1

    singleton_set_compound_term = NALGrammar.Terms.Term.from_string("[A]")
    singleton_set_compound_term_complexity = 2

    extensional_set_compound_term = NALGrammar.Terms.Term.from_string("{A,B}")
    # becomes an intensional intersection of sets, (|,{A},{B})
    extensional_set_compound_term_complexity = 5

    singleton_set_internal_compound_term = NALGrammar.Terms.Term.from_string("[(*,A,B)]")
    singleton_set_internal_compound_term_complexity = 4

    statement_term = NALGrammar.Terms.Term.from_string("(A-->B)")
...
def array_term_indexing_test():
    array_term_name = "M"
    array_term = NALGrammar.Terms.ArrayTerm(name=array_term_name, dimensions=(5, 5)) # create a 5x5 array term
