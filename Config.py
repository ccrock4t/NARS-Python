...
NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_CONCEPT = user_config["NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_CONCEPT"] # The number of times to look for a semantically related concept to interact with
NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_BELIEF = user_config["NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_BELIEF"] # The number of times to look for a semantically related belief to interact with
PRIORITY_DECAY_MULTIPLIER = user_config["PRIORITY_DECAY_MULTIPLIER"] # value in [0,1] multiplied w/ priority during priority decay
PRIORITY_STRENGTHEN_VALUE = user_config["PRIORITY_STRENGTHEN_VALUE"] # priority strengthen bor multiplier when concept is activated

TASKS_PER_CYCLE = user_config["TASKS_PER_CYCLE"]

WORKING_CYCLE_DELAY = user_config["WORKING_CYCLE_DELAY"] # in milliseconds
...
ARRAY_SENTENCES_DRAW_INDIVIDUAL_ELEMENTS = user_config["ARRAY_SENTENCES_DRAW_INDIVIDUAL_ELEMENTS"] # whether or not to draw each individual element / pixel of an array sentence. Turning this to False results in GUI speedup when viewing array sentences
...
"""
    Other Structures
"""
MAX_EVIDENTIAL_BASE_LENGTH = user_config["MAX_EVIDENTIAL_BASE_LENGTH"] # maximum IDs to store documenting evidential base
