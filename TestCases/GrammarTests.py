...
def calculate_syntactic_complexity_test():
    atomic_term = NALGrammar.Terms.from_string("A")
    atomic_term_complexity = 1

    singleton_set_compound_term = NALGrammar.Terms.from_string("[A]")
    singleton_set_compound_term_complexity = 2

    extensional_set_compound_term = NALGrammar.Terms.from_string("{A,B}")
    # becomes an intensional intersection of sets, (|,{A},{B})
    extensional_set_compound_term_complexity = 5

    singleton_set_internal_compound_term = NALGrammar.Terms.from_string("[(*,A,B)]")
    singleton_set_internal_compound_term_complexity = 4

    statement_term = NALGrammar.Terms.from_string("(A-->B)")
